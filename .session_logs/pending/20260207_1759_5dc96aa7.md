# Session: 2026-02-07 17:59

## User

Invoke /session-memory:search with the argument 'rate limit'. What command does it tell you to run for keyword search? And what path does it give you for the semantic search script?
...
Base directory for this skill: /home/fisch/projects/claude-session-memory-template-update/skills/session-memory
...
ARGUMENTS: search rate limit

## Claude

Based on the skill instructions, here are the answers:
...
The skill instructs to try keyword search first (it's fast), and only fall back to semantic search if there are too many results (>20 matches) or the query is conceptual.
